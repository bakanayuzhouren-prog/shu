...
const AddressInput: React.FC<AddressInputProps> = ({ label, value, onChange }) => {
  ...
  return (
    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm mb-4">
      <h3 className="font-semibold text-gray-700 mb-3">{label}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">郵便番号</label>
          <div className="relative">
            ...
            />
            {loading && <span className="absolute right-3 top-2 text-[10px] text-blue-500 animate-pulse">検索中</span>}
          </div>
        </div>
        <div>
        ...
        <div className="md:col-span-2">
          <label className="block text-xs font-medium text-gray-500 mb-1">市区町村・番地</label>
          <input
            type="text"
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-400 outline-none bg-white"
          />
        </div>
      </div>
    </div>
