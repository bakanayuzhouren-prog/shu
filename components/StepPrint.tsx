...
const StepPrint: React.FC<StepPrintProps> = ({ data, onBack }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto text-center space-y-8 flex flex-col items-center">
      <div className="no-print text-gray-500 text-sm">
      ...
      >
         <div className={`${imageContainerHeight} bg-gray-50 overflow-hidden flex-shrink-0`}>
            {data.processedImage && <img src={data.processedImage} className="w-full h-full object-cover" />}
         </div>
         <div className="flex-1">
            <h2 className={`${titleClass} font-serif font-bold text-gray-800`}>{data.name} より</h2>
            <p className={`${textClass} font-serif whitespace-pre-wrap text-gray-700`}>{data.customMessage}</p>
         </div>
      ...
      <div className="no-print space-x-4">
        <button onClick={onBack} className="px-6 py-2 border rounded">デザインに戻る</button>
        <button onClick={() => window.print()} className="px-10 py-3 bg-blue-600 text-white rounded-lg font-bold text-xl shadow-lg">🖨️ 印刷 / PDF保存</button>
      </div>
    </div>
  );
};
