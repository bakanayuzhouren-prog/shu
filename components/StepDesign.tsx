...
const StepDesign: React.FC<StepDesignProps> = ({ data, updateData, onNext, onBack }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
      <div className="space-y-6">
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
          <h2 className="font-bold text-lg mb-4 flex items-center gap-2 text-gray-800">
            <span className="text-purple-600">✍️</span> 1. 挨拶文を生成
          </h2>
          <p className="text-sm text-gray-500 mb-4">AIがあなたの趣味や新居に合わせて、温かい文章を提案します。</p>
          <button 
            onClick={handleAiText} 
            disabled={isGenerating} 
            className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-4 rounded-xl mb-4 font-bold shadow-lg hover:shadow-purple-200 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isGenerating ? (
          ...
          <textarea 
            ...
            className="w-full border-gray-200 border h-48 p-4 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none text-gray-700 leading-relaxed text-sm"
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
          <h2 className="font-bold text-lg mb-4 flex items-center gap-2 text-gray-800">
            <span className="text-pink-600">📸</span> 2. 写真を加工・編集
          </h2>
          <p className="text-sm text-gray-500 mb-4">写真をアップロードして、イラスト化したり、AIに指示して自由に編集できます。</p>
          <div className="space-y-4">
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50 transition bg-gray-50/50">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-pink-50 rounded-xl border border-pink-100 space-y-3">
                  ...
                  <button 
                    ...
                    disabled={isGenerating} 
                    className="w-full bg-pink-600 text-white py-3 rounded-xl font-bold shadow-md hover:bg-pink-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                ...
                <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 border border-indigo-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 outline-none bg-white"
                      disabled={isGenerating}
                    ...
                    <button
                      ...
                      className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold shadow-md hover:bg-indigo-700 transition-all disabled:opacity-50 whitespace-nowrap text-sm"
                    >
                      実行
                    </button>
                  </div>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 flex flex-col items-center lg:sticky lg:top-24 h-fit">
        <h2 className="font-bold text-gray-400 text-sm mb-6 uppercase tracking-widest">
        ...
        >
           <div className="flex-1 bg-gray-50 mb-8 overflow-hidden rounded-lg border border-gray-100 relative shadow-inner group">
              {data.processedImage ? (
                <>
           ...
           {/* Decorative Elements */}
           <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-br from-blue-500/5 to-transparent rounded-bl-full pointer-events-none"></div>
        </div>

        <div className="flex justify-between w-full mt-10">
          <button onClick={onBack} className="flex items-center gap-1 text-gray-400 hover:text-gray-600 font-bold transition">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 19l-7-7 7-7"></path></svg>
            戻る
          </button>
          <button 
            onClick={onNext} 
            className="bg-green-600 text-white px-12 py-4 rounded-xl font-bold shadow-xl hover:bg-green-700 hover:-translate-y-1 transition-all active:translate-y-0"
          >
            完成！印刷へ進む
          </button>
        </div>
