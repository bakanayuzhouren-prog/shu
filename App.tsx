...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-50 no-print border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setStep('input')}>
            <span className="text-2xl">🏠</span>
            <h1 className="font-bold text-lg text-gray-800">AI 引っ越し挨拶状メーカー</h1>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setShowHostingGuide(true)} className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition">
              <span>⚙️</span> <span className="hidden sm:inline">ガイド</span>
            </button>
            <button onClick={() => setShowPinSetup(true)} className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition">
